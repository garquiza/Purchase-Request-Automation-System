...
$sql = "SELECT * FROM bac_users";
$stmt = $pdo->prepare($sql);
$stmt->execute();
$bac_users = $stmt->fetchAll(PDO::FETCH_ASSOC);

$items_per_page = 20;
$total_users = count($bac_users);
            ...
            <div class="table-responsive">
                <table class="table table-striped" id="userTable">
                    ...
                    <tbody>
                        <?php if (count($paginated_users) > 0): ?>
                            <?php foreach ($paginated_users as $user): ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php
                                        $permissions = explode(',', $user['permission_access']);
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="7" class="text-center">No BAC users found.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
        ...
        function confirmDelete(userId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch(`src/process/delete_bac.php`, {
                            method: 'POST',
                            headers: {
